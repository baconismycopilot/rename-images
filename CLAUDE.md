...
`--editable` means the installed `rename-images` command always reflects the current state of this checkout — no reinstall needed after editing `rename_images.py`. The command lands in `~/.local/bin` (on `PATH` already via `uv tool install`) and works from any directory, not just this repo. Note that editable only tracks *code*: adding a new dependency to `pyproject.toml` requires `uv tool upgrade --reinstall rename-images` before the installed command sees it (the tool's env is resolved at install time, separately from the repo's `.venv`).
...
`generate_remote()` (in `rename_images.py`) POSTs to `<remote-url>/api/generate` with the prompt, the image as base64, and `num_predict` set from `--max-tokens` — the HTTP side is only `urllib`/`base64` from the stdlib, so this path adds no networking dependency. The image *bytes* it sends aren't always the raw file, though: Ollama's server-side image loader only decodes JPEG/PNG (`_REMOTE_NATIVE_SUFFIXES`) and rejects anything else with an HTTP 400 "Failed to load image or audio file" — HEIC (i.e. every unconverted iPhone photo) most notably. `_remote_image_bytes()` passes JPEG/PNG through untouched and transcodes every other supported extension to an in-memory JPEG (quality 90) first, applying `ImageOps.exif_transpose()` so orientation survives the fact that the transcoded copy carries no EXIF — a side effect being that GPS and other metadata never leave the machine for those formats. The file on disk is never modified. See "HEIC support" below for how Pillow gets the ability to decode HEIC at all. `-m/--model` is repurposed as the Ollama model tag when `-u` is set (default `qwen2.5vl:7b`, defined as `DEFAULT_REMOTE_MODEL`); make sure whatever tag you pass has been `ollama pull`ed on the remote machine first. HTTP errors from Ollama (e.g. a 404 for an unpulled model) have their JSON error body surfaced in the skip message, not just the generic status text — so a per-image failure is still self-diagnosing even after the preflight check below has passed.
...
Stock Pillow has no HEIC codec (licensing), so before `pillow-heif` was added, every `.heic` file — i.e. every unconverted iPhone photo — hit two failure modes at once:

1. **Loud, remote:** `generate_remote()` shipped the raw HEIC bytes to Ollama, whose image loader only decodes JPEG/PNG, producing an HTTP 400 `"Failed to load image or audio file"` per image. (This looked like a model limitation but isn't — the request dies in Ollama's image decoding, upstream of whatever model is loaded.)
2. **Silent, everywhere:** `get_photo_metadata()`'s `Image.open()` raised, was swallowed by its intentional catch-all fallback, and the image got file-creation-time dates and an empty EXIF dict — so HEIC filenames silently carried the wrong date whenever ctime ≠ shot time (AirDrop, copies), and the `exif` command showed nothing.

The fix is `register_heif_opener()` from [`pillow-heif`](https://pypi.org/project/pillow-heif/) (a regular `pyproject.toml` dependency; small, prebuilt wheels), called once at module import. Registration is process-global, so *everything* downstream gains HEIC for free: EXIF/date extraction, the `exif` command, the local MLX path (mlx-vlm loads images through PIL in this same process), and the remote path's transcoding (`_remote_image_bytes()` — see "Remote backend" above). Two details worth knowing: `pillow-heif` normalizes EXIF orientation itself on load (the tag reads `1` and pixels are already upright, so the transcode's `exif_transpose()` is a no-op for HEIC but still matters for e.g. TIFF/WebP), and HEIC files are renamed in place keeping their `.heic` extension — nothing converts the file on disk.
...
- **2026-07-18** — Added HEIC support via `pillow-heif` (`register_heif_opener()` at import). Diagnosed from a real run's per-image `HTTP 400 "Failed to load image or audio file"` on every `.HEIC` file: not a model limitation but Ollama's server-side image loader, which only decodes JPEG/PNG. `generate_remote()` now transcodes non-JPEG/PNG formats to in-memory JPEG via a new `_remote_image_bytes()` helper (orientation baked in with `exif_transpose()`; files on disk untouched). The same missing codec was also *silently* breaking HEIC EXIF/date extraction everywhere — `Image.open()` raised, `get_photo_metadata()`'s fallback swallowed it, and iPhone photos got file-ctime dates and empty EXIF — which registration fixes for the local path, the cache, and the `exif` command in one shot. See "HEIC support" above. Also fixed two env papercuts hit along the way: stale `.venv` script shebangs from the repo having moved (`uv sync --reinstall`) and the `uv tool` env not picking up new deps under an editable install (`uv tool upgrade --reinstall rename-images`, now noted in Setup).
